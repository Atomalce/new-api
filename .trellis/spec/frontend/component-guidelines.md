...
- 本规范约束扁平化后的 `web/`(React 19 + TypeScript + Rsbuild + Base UI + Tailwind CSS v4)。旧的 `web/default` 与 `web/classic` 已移除。
- 一律使用 React 函数组件 + Hooks,单一职责;单文件超过约 200 行时拆分子组件或抽自定义 Hook(见 hook-guidelines.md)。
- 上位规范:根 `AGENTS.md` 与 `web/AGENTS.md`,本文与其冲突时以它们为准。
- 改动 `.ts`/`.tsx` 后必须在 `web/` 下执行 `bun run typecheck`(tsgo)与 `bun run lint`(oxlint),error 清零。
...
| 位置 | 内容 |
| --- | --- |
| `web/src/components/ui/` | shadcn 风格 registry 基础组件(Base UI + cva),由 `components.json` 管理,只封装样式与交互,无业务逻辑 |
| `web/src/components/` | 项目级公共组件:`confirm-dialog.tsx`、`status-badge.tsx`、`multi-select.tsx`、`data-table/`、`layout/` 等 |
| `web/src/features/<feature>/components/` | 功能组件(表格列、行操作、弹窗、抽屉),只服务本 feature |
| `web/src/lib/` | 通用工具(`utils.ts` 的 `cn()`、`api.ts` 等) |
| `web/src/styles/` | 全部自定义 CSS:`index.css`、`theme.css`、`theme-presets.css` |

归属判定遵循 `web/src/components/data-table/README.md`:
...
- feature 组件以 feature 名作前缀:`ApiKeysTable`、`ApiKeysPrimaryButtons`、`ApiKeysDeleteDialog`(`web/src/features/keys/components/`)。
...
- 每个组件的 props 必须有显式类型。公共组件示例(`web/src/components/confirm-dialog.tsx`):
...
- feature 弹窗/抽屉统一受控三件套:`open` + `onOpenChange` + 可选 `currentRow`(有 `currentRow` 即编辑态,`const isUpdate = !!currentRow`),见 `web/src/features/keys/components/api-keys-mutate-drawer.tsx`:
...
- 可复用展示组件必须透传 `className` 并用 `cn()` 合并;包装 DOM 元素时 `...props` 透传剩余原生属性,ui 基础组件加 `data-slot` 标识(见 `web/src/components/ui/button.tsx` 的 `data-slot='button'`)。
- 遵循 `web/AGENTS.md`:props 非必要不解构,整体接收 `props` 后按需访问/局部解构(如 `ConfirmDialog`、`StatusBadgeList`)。
...
- 包装原生事件时先做组件行为,再链式调用外部回调(`web/src/components/status-badge.tsx`):
...
- feature 内多个弹窗/抽屉的开关状态不逐层传 props,统一放 feature provider,消费方通过 hook 触发(`web/src/features/keys/components/api-keys-primary-buttons.tsx`):
...
- 数据变更走 React Query `useMutation`,成功后 `invalidateQueries` + `toast`;服务端错误统一 `handleServerError`(`web/src/lib/handle-server-error.ts`)。

---

## Styling (Tailwind / Theme)

- 只用 Tailwind 工具类;动态/条件类名必须经 `cn()`(clsx + tailwind-merge,`web/src/lib/utils.ts`)合并,禁止手写字符串拼接。
- 多变体组件用 `cva` 定义 `variant`/`size`,配 `VariantProps` 推导类型(`web/src/components/ui/button.tsx`):
...
- 颜色只用语义 token(`bg-primary`、`text-muted-foreground`、`bg-destructive/10` 等),token 由 `web/src/styles/theme.css` 的 `@theme inline` + CSS 变量定义(多主题预设在 `theme-presets.css`)。不要写死 `bg-blue-500` 之类的调色板色。
...
- 用户可见文案必须 `useTranslation()` + `t('English key')`,key 即英文原文,locale 文件为 `web/src/i18n/locales/{lang}.json` 的 flat JSON。真实示例:`t('Create API Key')`(`api-keys-primary-buttons.tsx`)。
...
- **手改 `web/src/routeTree.gen.ts`** —— 由 `@tanstack/router-plugin` 构建时自动生成(`rsbuild.config.ts`)。
